...
public class KingdomSelect : MonoBehaviour {
    ...
    public SpriteRenderer MapHolder;
    ...
    private void ChangeMap(Kingdom k)
    {
        MapHolder.GetComponent<SpriteRenderer>().sprite = k.Pic;
        MapHolder.DOFade(1, 0.2f);
    }
    public void LookAtKingdom(Kingdom k)
    {
        ...
        if (MapHolder.GetComponent<SpriteRenderer>().sprite != null)
        {
            MapHolder.DOFade(0, 0.2f).OnComplete(() => ChangeMap(k));
        }
        else
        {
            ChangeMap(k);
